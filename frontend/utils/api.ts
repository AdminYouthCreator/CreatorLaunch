...
export const authAPI = {
  ...
  register: async (userData: {
    ...
    const response = await api.post('/api/auth/register', {
      ...
      dob: userData.dob,
      parentEmail: userData.guardianEmail,
      parentalConsent: !!userData.guardianEmail,
    });
...
export const brandAPI = {
  ...
  update: async (_brandId: string, _brandData: any) => {
...
export const todoAPI = {
  ...
  // Update todo
  update: async (_todoId: string, todoData: any) => {
    const response = await api.patch('/api/todo', todoData);
    return response.data;
  },

  // Delete todo - Note: Backend doesn't have this endpoint
  delete: async (_todoId: string) => {
...
export const uploadAPI = {
  ...
  uploadLogo: async (file: File) => {
    ...
    const response = await api.put('/api/brands/logo', formData, {
      headers: {
  ...
  uploadFile: async (_file: File, _type?: string) => {
...
export const storeAPI = {
  ...
  getStoreProducts: async (storeUrl: string) => {
    const response = await api.get(`/api/store/${storeUrl}/products`);
    return response.data;
  },
...
export const orderAPI = {
  ...
  getAll: async (page?: number, status?: string) => {
    ...
    const response = await api.get(query ? `/api/orders?${query}` : '/api/orders');
    return response.data;
  },
